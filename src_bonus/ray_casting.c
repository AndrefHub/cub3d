...
void	draw_wall_scaled(t_img *img, const t_img *texture, const struct s_column *column, int x)
{
	...
	const unsigned	tex_col = (unsigned ) column->texture_pos % TEXTURE_SIZE;//texture->size.x;
//	const double	step = (double ) texture->size.y / column->height;
//	const unsigned	tex_col = (unsigned ) column->pos.x % texture->size.x;
	...
	while (y < max_height)
	{
		img->addr[y * img->size.x + x] = texture->addr[(unsigned )tex_y * TEXTURE_SIZE + tex_col];
		tex_y += step;
		y++;
	}
}
...
void	draw_walls(t_game *game)
{
	...
	while (x < game->img.size.x)
	{
		...
		draw_wall_scaled(&game->img, &game->textures[0],
						 &game->column[x], x);
		x++;
	}
}

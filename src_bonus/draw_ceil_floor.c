...
static void	draw_row_textured(t_game *game, t_img src, t_vector cur, float row_distance)
{
	...
	const t_fvector	ray_dir1 = (t_fvector){(float)(game->player.vector.x + game
			->player.plane.x), (game->player.vector.y + game->player.plane.y)};
	...
	while (cur.x < game->img.size.x)
	{
		...
		if (MAX_RENDER_DISTANCE >= row_distance)
			game->img.addr[cur.y * game->img.size.x + cur.x] = src.addr[(tex.y
					* TEXTURE_SIZE + tex.x) % (TEXTURE_SIZE * TEXTURE_SIZE)];
		cur.x++;
	}
}
...
void	draw_ceil_floor_textured(t_game *game)
{
	...
	{
		row_distance = (0.5f * (float) game->col_scale) / ((float)(cur.y
					- (game->img.size.y / 2 - game->z_offset)));
		if (row_distance < 0)
			draw_row_textured(game, *((t_img *)game->map->ceiling.img->content),
				cur, -row_distance);
		else
			draw_row_textured(game, *((t_img *)game->map->floor.img->content),
				cur, row_distance);
		cur.y++;
	}
}

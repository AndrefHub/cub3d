...
void	death_message(t_game *game)
{
	...
	put_text_to_screen_layout(game->map->font, &game->img, &(t_text){"YOU DIED!", (t_vector)
	{game->img.size.x / 2, game->img.size.y / 2 - font_size / 2},
		VCenter | HCenter, 0x7F7FFF}, font_size);
	put_text_to_screen_layout(game->map->font, &game->img, &(t_text){game->hud.score.value,
		(t_vector){game->img.size.x / 2, game->img.size.y / 2 + font_size / 2},
		VCenter | HCenter, 0x7FFF7F}, font_size);
	put_text_to_screen_layout(game->map->font, &game->img, &(t_text){"username:", (t_vector)
...
void	put_username_on_screen(t_game *game)
{
	...
	put_text_to_screen_layout(game->map->font, &game->img, &(t_text){game->username, pos,
